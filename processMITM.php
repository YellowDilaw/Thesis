...
if ($score >= $passingScore) {
    ...
    echo '<br><br><a href="certMITM.php" class="cert-button" target="_blank">View Certificate</a>';
    $scoreformat = ';' . $score;
    $user_id = $results['id'];
    ...
    $query = "UPDATE courses SET MITMstat = 1 WHERE user_id = {$results['id']};";
    $result = mysqli_query($conn, $query);

    //STORE SCORE IN DATABASE
    // Check the current value of MITM_prog
    $sql = "SELECT mitm_quiz_score FROM progress WHERE user_id = $user_id";
    $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                
            $row = $result->fetch_assoc();
            $existingScore = $row['mitm_quiz_score'];
            $setScore = $existingScore . $scoreformat;
            // Update the user's score in the database (assuming a user is logged in)

            // If a row with the user_id exists, update mitm_quiz_score
            $sql = "UPDATE progress SET mitm_quiz_score = '$setScore' WHERE user_id = $user_id";
            ...
            if ($conn->query($sql) === TRUE) {
                ...
                $sql = "INSERT INTO progress (mitm_quiz_score ,user_id) VALUES ('$scoreformat', $user_id)";
...
<button type="button" onClick="showScore()"> Click to see your attempts </button>
    ...
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $currentValue = $row['mitm_quiz_score'];
