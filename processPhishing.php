...
if ($score >= $passingScore) {
    ...
    echo '<br><br><a href="certPhishing.php" class="cert-button" target="_blank">View Certificate</a>';
    $scoreformat = ';' . $score;
    $user_id = $results['id'];
    ...
    $query = "UPDATE courses SET Phishingstat = 1 WHERE user_id = {$results['id']};";
    $result = mysqli_query($conn, $query);

    //STORE SCORE IN DATABASE
    // Check the current value of phishing_prog
    $sql = "SELECT phish_quiz_score FROM progress WHERE user_id = $user_id";
    $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                
            $row = $result->fetch_assoc();
            $existingScore = $row['phish_quiz_score'];
            $setScore = $existingScore . $scoreformat;
            // Update the user's score in the database (assuming a user is logged in)

            // If a row with the user_id exists, update phish_quiz_score
            $sql = "UPDATE progress SET phish_quiz_score = '$setScore' WHERE user_id = $user_id";
            ...
            if ($conn->query($sql) === TRUE) {
                ...
                $sql = "INSERT INTO progress (phish_quiz_score ,user_id) VALUES ('$scoreformat', $user_id)";
...
<button type="button" onClick="showScore()"> Click to see your attempts </button>
    ...
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $currentValue = $row['phish_quiz_score'];
